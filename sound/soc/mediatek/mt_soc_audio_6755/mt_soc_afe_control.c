...
//EnableALLbySampleRate MCLK  modify by MTK oliverchen 2060314
void EnableALLbySampleRate(uint32 SampleRate)
{
	...
	if ((GetApllbySampleRate(SampleRate) == Soc_Aud_APLL1)
	    && ((APLLUsage == Soc_Aud_APLL_NOUSE) || APLLUsage == Soc_Aud_APLL1)) {
		/* enable APLL1 */
		APLLUsage = Soc_Aud_APLL1;
		APLLCounter++;
		pr_debug("APLLCounter = %d APLLUsage = %d++++\n", APLLCounter,APLLUsage);
		if (APLLCounter == 1) {
		    pr_debug("APLLCounter == true++\n");
			AudDrv_Clk_On();
			EnableApll1(true);
		   ...
		   && (APLLUsage == Soc_Aud_APLL_NOUSE || APLLUsage == Soc_Aud_APLL2)) {
		/* enable APLL2 */
...
uint32 SetCLkMclk(uint32 I2snum, uint32 SampleRate)
{
	...
	case Soc_Aud_I2S0:
		...
		Afe_Set_Reg(AUDIO_CLK_AUDDIV_0, 0x0 << 2, 0x1 << 2); //AAAA Add this line

		break;
	case Soc_Aud_I2S1:
		I2s_ck_div = (I2S_APll / MCLKFS / SampleRate) - 1;
		Afe_Set_Reg(AUDIO_CLK_AUDDIV_1, I2s_ck_div << 8, 0x0000ff00);
		Afe_Set_Reg(AUDIO_CLK_AUDDIV_0, 0x0 << 3, 0x1 << 3); //AAAA Add this line

		break;
	case Soc_Aud_I2S2:
		I2s_ck_div = (I2S_APll / MCLKFS / SampleRate) - 1;
		Afe_Set_Reg(AUDIO_CLK_AUDDIV_1, I2s_ck_div << 16, 0x00ff0000);
		Afe_Set_Reg(AUDIO_CLK_AUDDIV_0, 0x0 << 4, 0x1 << 4); //AAAA Add this line

		break;
	case Soc_Aud_I2S3:
		I2s_ck_div = (I2S_APll / MCLKFS / 48000) - 1;//set mclk fix 6.14MHZ modify oliverchen 20160314
		Afe_Set_Reg(AUDIO_CLK_AUDDIV_1, I2s_ck_div << 24, 0xff000000);
		//set mclk set by oliver chen 20160314 begin
		Afe_Set_Reg(AUDIO_CLK_AUDDIV_0, 0x0 << 5, 0x1 << 5); //AAAA Add this line
	       //set mclk set by oliver chen 20160314 end
