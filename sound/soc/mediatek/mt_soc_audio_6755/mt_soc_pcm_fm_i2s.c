...
// stop I2S output
    ...
    // FM speaker function add start by oliverchen 20151013
    SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2, false);
    if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2) == false)
    {
        Afe_Set_Reg(AFE_I2S_CON3, 0x0, 0x1);
    }
    //end by oliverchen 20151013
    #endif
	...
	DisableALLbySampleRate(substream->runtime->rate);//add oliver
	AudDrv_I2S_Clk_Off();
	AudDrv_Clk_Off();
	mPrepareDone = false;
...
static int mtk_pcm_fm_i2s_prepare(struct snd_pcm_substream *substream)
{
	AudioDigtalI2S m2ndI2SInAttribute;
		#ifdef CONFIG_SND_SOC_TAS2555
	uint32 u32AudioI2S = 0;// FM speaker function add  by oliverchen 20151013
	uint32 MclkDiv3;
	#endif
	struct snd_pcm_runtime *runtime = substream->runtime;
	...
	if (mPrepareDone == false) {
     ...
     //end by oliverchen 20151013
		...
		// I2S out Setting
		 ...
		 //end by oliverchen 20151013
		 //-------------------------------------start mclk set by oliver chen 20160314
    ...
    //------------------------------------------end  mclk set by oliver chen 20160314
