...
static int mtk_voice_close(struct snd_pcm_substream *substream)
{
	...
	//add start by oliver  for speech speaker 20151020
	...
	//add end by oliver  for speech speaker 20151020
	#endif
	...
	//EnableI2SDivPower(AUDIO_APLL12_DIV2, false);//add mtk
...
static int mtk_voice1_prepare(struct snd_pcm_substream *substream)
{
	...
	#ifdef CONFIG_SND_SOC_TAS2555
	uint32 MclkDiv3;
    uint32 u32AudioI2S = 0;	//add start by oliver  for speech speaker 20151020
   // unsigned int md1_SampleRate=0;
	#endif
	...
	// I2S out Setting
    u32AudioI2S = SampleRateTransform(runtimeStream->rate) << 8;
    u32AudioI2S |= Soc_Aud_I2S_FORMAT_I2S << 3; // us3 I2s format
    u32AudioI2S |= Soc_Aud_I2S_WLEN_WLEN_32BITS << 1; //32bit
	//-------------------------------------mclk set by oliver
	/* here to open APLL */
	
	EnableALLbySampleRate(runtimeStream->rate);
	MclkDiv3=SetCLkMclk(Soc_Aud_I2S1, runtimeStream->rate); /* select I2S */
	MclkDiv3=SetCLkMclk(Soc_Aud_I2S3, runtimeStream->rate); /* select I2S */
	SetCLkBclk(MclkDiv3,  runtimeStream->rate, runtimeStream->channels,
    ...
    if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2) == false)
    {
		...
		pr_warn("set AFE_I2S_CON3\n");
		
    }
    else
    {
        SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2, true);
		pr_warn("setMemoryPathEnable I2S_OUT_2\n");
    }
    //add end by oliver  for speech speaker 20151020
#endif
