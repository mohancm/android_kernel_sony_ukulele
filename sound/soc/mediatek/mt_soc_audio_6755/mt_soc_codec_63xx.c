...
#ifdef CONFIG_SND_SOC_TAS2555
 uint32 mBlockSampleRate[AUDIO_ANALOG_DEVICE_INOUT_MAX] = { 48000, 48000, 48000 };//for extern
#else
...
static void HP_Ana_Switch_to_On(void)
{
	...
	//zihwei modify for camera key 2015/11/19 begin
	...
	//zihwei modify for camera key 2015/11/19 end
#else
...
bool OpenHeadPhoneImpedanceSetting(bool bEnable)
{
	...
	if (bEnable == true) {
		...
		/* De_OSC of HP and output STBENH enable(470Ohm connect) */
...
#ifdef CONFIG_SND_SOC_TAS2555
 int mt63xx_codec_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *Daiport)//for extern
#else
...
#if 0  //TI smart pa tas2555 remove DAPM by oliverchen
	#ifdef CONFIG_SND_SOC_TAS2555
   extern struct snd_soc_pcm_runtime *rtd_speaker_control;//fix speaker and headset playback at the same time by oliverchen 
   #endif
#endif
static void Audio_Amp_Change(int channels, bool enable)
{
...
static int Audio_AmpR_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	#if 0  //TI smart pa tas2555 remove DAPM by oliverchen
	    #ifdef CONFIG_SND_SOC_TAS2555
	     printk("Audio_AmpR_Set-Close I2S3---MTK_Debug/n"); 
...
static void Speaker_Amp_Change(bool enable)
{
	...
	#if 0  //TI smart pa tas2555 remove DAPM by oliverchen
	#ifdef CONFIG_SND_SOC_TAS2555
	//fix speaker and headset playback at the same time by oliverchen start
	Afe_Set_Reg(AFE_I2S_CON3, 0x1, 0x1);
	printk("%s()---snd_soc_dapm_stream_event-START-tas2555\n",__func__);
      snd_soc_dapm_stream_event(rtd_speaker_control, 0,
      SND_SOC_DAPM_STREAM_START);
      //fix speaker and headset playback at the same time by oliverchen  end
	  #endif
	#endif
		if (GetDLStatus() == false)
			TurnOnDacPower();
	...
	#ifdef CONFIG_SND_SOC_TAS2555
	//fix speaker and headset playback at the same time by oliverchen start
	printk("%s()---snd_soc_dapm_stream_event-STOP-tas2555\n",__func__);
      snd_soc_dapm_stream_event(rtd_speaker_control, 0,
