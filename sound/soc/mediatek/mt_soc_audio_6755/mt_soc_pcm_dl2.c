...
//add fast playback by oliverchen 20151216
	SetConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I07,
...
static int mtk_soc_pcm_dl2_close(struct snd_pcm_substream *substream)
{
	...
	//	EnableI2SDivPower(AUDIO_APLL12_DIV2, false);//add mtk
		DisableALLbySampleRate(substream->runtime->rate);//oliver
		mPrepareDone = false;
	}
...
#ifdef CONFIG_SND_SOC_TAS2555
	uint32 u32AudioI2S = 0;//add fast playback by oliverchen 20151216
	uint32 MclkDiv3;
#endif
	struct snd_pcm_runtime *runtime = substream->runtime;
