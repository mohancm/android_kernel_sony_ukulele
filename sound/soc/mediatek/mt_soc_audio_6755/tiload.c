...
//    struct i2c_client *i2c = g_codec->control_data;
	...
	if (!g_TAS2555)
	{
		printk(KERN_ERR "tiload_read: no device\n\r");
		return -1;
	}

	if (!g_TAS2555->read)
	{
		printk(KERN_ERR "tiload_read: no single byte read function\n\r");
		return -1;
	}

	if (!g_TAS2555->bulk_read)
	{
		printk(KERN_ERR "tiload_read: no bulk read function\n\r");
		return -1;
	}
	...
	printk(KERN_ERR "tiload_read: copy register address from user space\n\r");
	...
	if (count == 1) {
		printk(KERN_ERR "tiload_read: execute single byte read\n\r");
		ret =
			g_TAS2555->read(g_TAS2555, 0x80000000 | nCompositeRegister, &Value);
		if (ret >= 0)
			rd_data[0] = (char) Value;
	} else if (count > 1) {
		printk(KERN_ERR "tiload_read: execute multi byte read\n\r");
		ret =
...
//    size = i2c_master_recv(i2c, rd_data, count);
	...
	printk(KERN_ERR "tiload_read: copy to user space\n\r");
	...
	printk(KERN_ERR "tiload_read: done\n\r");
