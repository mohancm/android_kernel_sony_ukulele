...
static unsigned char CTPM_FW[] = {
	#include "FT5x46i_FT34x7_V04_D03_20160615_app.i"
};

static unsigned char aucFW_PRAM_BOOT[] = {
	#include "FT5x46i_FT34x7_V03_D03_20160601_app.i"
};
...
{	
	...
	int i_ret;
	u8 ver;
	printk("zihweishen enther fts_5x46_ctpm_fw_upgrade\n");
		...
		for (i = 0; i < temp; i++) 
	{
		...
		for(i = 0;i < 30;i++)
		{
			...
			printk("1111[FTS][%s] reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
			msleep(1);
	...
	for(i = 0;i < 100;i++)
	{
		...
		if (0xF0==reg_val[0] && 0x55==reg_val[1])
		{
			dev_err(&client->dev, "11111[FTS]--reg_val[0]=%02x reg_val[0]=%02x\n", reg_val[0], reg_val[1]);
			break;
		}
		msleep(1);
...
/*[SM20][zihweishen] store touch vendor and firmware version to tpd_show_vendor_firmware 2016/06/20 begin*/ 
		#if defined(TPD_REPORT_VENDOR_FW)
			{
				...
				char buf[80]={0};
				//fts_i2c_read(fts_i2c_client, buf, 1, buf, 33)
				fts_read_reg(fts_i2c_client, 0xA6, &ver);
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	printk("zihweishen enther fts_ctpm_fw_upgrade_with_app_file %s\n",firmware_name);
	...
	{
		i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, fwsize);
		printk("zihweishen enther fts_5x46_ctpm_fw_upgrade %x\n",fts_updateinfo_curr.CHIP_ID);
	}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
		...
		printk("[FTS][print1] uc_tp_fm_ver = 0x%x,  uc_chip_id= 0x%x\n",uc_tp_fm_ver, uc_chip_id);
	...
	{	
		...
		if (uc_tp_fm_ver == FTS_REG_FW_VER ||	uc_tp_fm_ver < uc_host_fm_ver ) 
		{
			...
			msleep(100);
			TPD_DEBUG("[print2] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
