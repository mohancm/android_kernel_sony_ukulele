...
static struct device_attribute dev_attr_##propname =\
		...
		concat(synaptics_rmi4_f54, _##propname##_show), synaptics_rmi4_store_error);
...
static struct device_attribute dev_attr_##propname =\
		...
		synaptics_rmi4_show_error,concat(synaptics_rmi4_f54, _##propname##_store));
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (ii = PDT_START; ii > PDT_END; ii -= PDT_ENTRY_SIZE) {
			...
			retval = f54->fn_ptr->read(rmi4_data,
					...
					sizeof(rmi_fd));
			if (retval < 0)
			printk("zihwei add log for detect system why into synaptics_rmi4_f54_init function \n");
				goto exit_free_f54; //[SM20][zihweishen] ALPS02691261 MTK provide solution for KE crash 2016/4/28
