...
//<2015/09/16-zihweishen, Porting HALL_sensor and Smart cover begin
#define HALL_SENSOR_ENABLE 0
...
#define HALL_POLARITY   mt_get_hall_sensor_gpio_value(59)


//<2015/11/30-zihweishen, Power key will not enable when LID closed begin
/*enum LID_STATE{
	LID_OPEN,
	LID_CLOSE,
};*/
...
//>2015/11/30-zihweishen, Power key will not enable when LID closed end

static void hall_handler(unsigned long data);
static DECLARE_TASKLET(hall_tasklet, hall_handler, 0);
static u8 chall_state =TRIGGER_LOW; //2015/11/30-zihweishen, When power on, hall sensor lead to LID close
//static u8 hall_state=1;
//static u8 ohall_state = TRIGGER_RISING;


//<2015/11/30-zihweishen, Power key will not enable when LID closed begin
static bool lid_state = 0;
//>2015/09/16-zihweishen, Power key will not enable when LID closed end
#endif
//>2015/11/30-zihweishen, Porting HALL_sensor and Smart cover end
...
//<2015/11/30-zihweishen, Porting HALL_sensor and Smart cover begin
/*********************************************************************/
...
struct of_device_id hall_of_match[] = {
	{ .compatible = "mediatek,mt6755-hall_1", },
	{},
};
...
printk("zihwei hall sensor4\n");
	if (hirq_flag == 0) {
		...
		spin_unlock_irqrestore(&hirq_flag_lock, flags);
		printk("HALL Eint already enabled!");
	} else {
		spin_unlock_irqrestore(&hirq_flag_lock, flags);
		printk("Invalid irq_flag %d!", hirq_flag);
	}
...
void hall_irq_disable(void)
{
	...
	spin_lock_irqsave(&hirq_flag_lock, flags);
printk("zihwei hall sensor3\n");
	if (hirq_flag == 1) {
		hirq_flag = 0;
		printk("zihwei hirq_flag = 0\n");
		spin_unlock_irqrestore(&hirq_flag_lock, flags);
		//disable_irq(hall_irq);
		disable_irq_nosync(hall_irq); 
	} else if (hirq_flag == 0) {
		spin_unlock_irqrestore(&hirq_flag_lock, flags);
		printk("HALL Eint already disabled!");
	} else {
		spin_unlock_irqrestore(&hirq_flag_lock, flags);
		printk("Invalid irq_flag %d!", hirq_flag);
	}
	printk("zihwei hall_irq_disable end!");
...
{
	//printk("[Steven] %s \n", __func__);
	printk("zihwei hall_eint_handler\n");
	//mt_eint_mask(CUST_EINT_HALL_1_NUM);
	hall_irq_disable();
	printk("zihwei hall_irq_disable\n");
	printk("zihwei GPIO59=%x\n",mt_get_hall_sensor_gpio_value(59));
	//printk("zihwei HALL_POLARITY=%x\n",HALL_POLARITY);
	tasklet_schedule(&hall_tasklet);
	printk("zihwei tasklet_schedule\n");
	return IRQ_HANDLED;
}
...
static int hall_irq_registration(void)
{
	...
	u32 ints[2] = { 0, 0 };
	//int false=0;
	printk("Device Tree hall_irq_registration!");
	printk("zihwei hall sensor1\n");
...
static void hall_handler(unsigned long data)
{
	...
	unsigned int hall_int59_num=59;
	u8 old_state = chall_state;
	printk("zihwei hall sensor2\n");
	
	//<2015/09/16-zihweishen, Power key will not enable when LID closed begin
	lid_state = (chall_state==TRIGGER_LOW)?1:0;//2015/11/30-zihweishen, When power on, hall sensor lead to LID close
	printk( "[Steven] Lid %s \n", (lid_state==1)?"close":"open");
	//>2015/09/16-zihweishen, Power key will not enable when LID closed end
	printk( "[zihwei] chall_state %x \n", chall_state);
	printk( "[zihwei] lid_state %d \n", lid_state);

	//chall_state=ohall_state;
	
	/* for SW_LID, 0: lid open => slid, 1: lid shut => closed */
	if (chall_state==TRIGGER_LOW)//2015/11/30-zihweishen, When power on, hall sensor lead to LID close
		{
		//irq_set_irq_type(gpio_to_irq(hall_int59_num), old_state);
	input_report_switch(kpd_input_dev, SW_LID, 0);
	input_sync(kpd_input_dev);
	printk("zihwei hall SW_LID, 0\n");
	//chall_state=ohall_state;
...
//>2015/09/16-zihweishen, Porting HALL_sensor and Smart cover end
...
//<2015/11/30-zihweishen, Power key will not enable when LID closed begin
#if HALL_SENSOR_ENABLE
	if(mt_get_hall_sensor_gpio_value(59)==0)
	{
		printk("[Steven] LID_CLOSE. Block power key action \n");
		printk("zihwei LID_CLOSE. Block power key action\n");
		return;
	}
//>2015/11/30-zihweishen, Power key will not enable when LID closed end
	printk("zihwei hall LID_CLOSE. Block power key action lid_state==%d\n",lid_state);
#endif
...
//<2015/09/16-zihweishen, Porting Smart cover begin
#if HALL_SENSOR_ENABLE
printk("zihwei hall sensor1\n");
	__set_bit(EV_SW, kpd_input_dev->evbit);
	__set_bit(SW_LID, kpd_input_dev->swbit);
#endif
//>2015/09/16-zihweishen, Porting Smart cover end
	...
	//<2015/11/25-zihweishen, Add Camera Key(AF:GPIO22/EINT22; Capture:GPIO24/EINT24) 
	input_set_capability(kpd_input_dev, EV_KEY, 212);	//add key_focus keycode 
	input_set_capability(kpd_input_dev, EV_KEY, 211);	//add key_camera keycode 
	//>2015/11/25-zihweishen 
...
//<2015/09/16-zihweishen, Porting HALL_sensor and Smart cover begin
#if HALL_SENSOR_ENABLE
printk("zihwei hall_irq_registration\n");
