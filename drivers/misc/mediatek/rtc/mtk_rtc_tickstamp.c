...
static void ts_read_stamp(struct work_struct *work) {
    ...
    //by major, wait system partition firstly, to prevent waiting for cache partition mounted by /sbin/mr
    //that may result in /sbin/mr failed to umount cache partition due to Device or resource is busy
