...
static int prepare_runtime_data(struct ccci_modem *md, struct ccci_request *req)
{
	...
	for (i = BOOT_INFO; i < FEATURE_COUNT; i++) {
		...
		if (rt_feature.support_info.support_mask == CCCI_FEATURE_MUST_SUPPORT) {
		    //by major, for test_case
		   CCCI_NOTICE_MSG(md->index, KERN, "prepare_runtime_data  rt_feature.feature_id =%u\n", rt_feature.feature_id);
			...
			case MISC_INFO_SBP_ID:
				...
				CCCI_NOTICE_MSG(md->index, KERN, "prepare_runtime_data  md->sbp_code=%u\n", md->sbp_code);
	//by major, fix issue about sbp nvram config not update after userdata is cleared
		...
		{  //sbp not setted, then to get it from proc
			...
			CCCI_NOTICE_MSG(md->index, KERN, "[kernel]md=%d, apsbp_id=%u got.  sbp_code=%u\n", md->index , apsbp_id,rt_f_element.feature[0]);
			
			if(apsbp_id < 1000)  //it is setted
			{
			      CCCI_NOTICE_MSG(md->index, KERN, "CDF sbp  got!!\n");
			      send_sbp_flag = 1;

			      if(rt_f_element.feature[0] != apsbp_id)
			      {
			           rt_f_element.feature[0] = apsbp_id;
			           CCCI_NOTICE_MSG(md->index, KERN, "sbp  changed...sbp_code:%u\n" , apsbp_id);
			      }
			}
			else
			{
			     CCCI_NOTICE_MSG(md->index, KERN, "CDF sbp  NOT setted!!\n");
			}
