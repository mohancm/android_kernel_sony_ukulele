...
extern ssize_t RearAFPos_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count);
...
static DEVICE_ATTR (RearAF,  0664, NULL,  RearAFPos_store);
static inline int Register_AF_CharDrv(void)
{
	...
	device_create_file(vcm_device, &dev_attr_RearAF);
