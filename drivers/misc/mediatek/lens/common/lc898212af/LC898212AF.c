...
static int s4EEPROM_ReadReg(u16 addr, u16 *data)
{
	...
	g_pstAF_I2Cclient->timing=400;//20160217 Rick Liu, AF performance improve
...
//<2015/10/04-kylechang, Add AF Driver LC898212
/*	 MTK Release
...
*/
	s4EEPROM_ReadReg(0x02E4, &posl);
	s4EEPROM_ReadReg(0x02E5, &posh);
	min_pos =  (posh<<8) + posl;
	s4EEPROM_ReadReg(0x02E6, &posl);
	s4EEPROM_ReadReg(0x02E7, &posh);
	max_pos =  (posh<<8) + posl;
	
	s4EEPROM_ReadReg(0x04A9, &eepdata1);
	No_eeprom = s4EEPROM_ReadReg(0x04A8, &eepdata2);
	//>2015/10/04-kylechang
	...
	//msleep(5);//20160217 Rick Liu, AF performance improve
	s4AF_ReadReg(0, 0x85, &Reg_0x85);
	while (Reg_0x85 != 0x00) {
		//msleep(1);
		mdelay(1);//20160217 Rick Liu, AF performance improve
		s4AF_ReadReg(0, 0x85, &Reg_0x85);
	}
	...
	//msleep(5);
	mdelay(5);//20160217 Rick Liu, AF performance improve

}

ssize_t RearAFPos_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	u16 TargetPos;
	/* u32 tmpcal=0; */
	u16 ExistentPos = 0;
	...
	//<2015/10/14 RickLiu MTK Release
		Sitch = 1023 - Sitch; //<2015/10/15 - RickLiu mark for AF issue
	//>2015/10/14 RickLiu MTK Release
	...
	if (Sitch < 0x800)
		TargetPos = 0x800 - Sitch;
	else
		TargetPos = (0x1800 - Sitch) & 0xFFF;
...
static int SetVCMPos(u16 _wData)
{
	...
	//<2015/10/14 RickLiu MTK Release
		_wData = 1023 - _wData; //<2015/10/15 - RickLiu mark for AF issue
	//>2015/10/14 RickLiu MTK Release
