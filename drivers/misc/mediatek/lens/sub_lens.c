...
extern ssize_t OIS_Switch_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count);
extern ssize_t OIS_Shutter_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count);
extern ssize_t OIS_Movie_store (struct device *dev,struct device_attribute *attr, const char *buf, size_t count);
extern ssize_t FrontAF_store (struct device *dev,struct device_attribute *attr, const char *buf, size_t count);
...
static DEVICE_ATTR (OIS_Switch,  0664, OIS_Switch_show,  OIS_Switch_store);
static DEVICE_ATTR (OIS_Shutter,  0664, NULL,  OIS_Shutter_store);
static DEVICE_ATTR (OIS_Movie,  0664, NULL,  OIS_Movie_store);
static DEVICE_ATTR (FrontAF,  0664, NULL,  FrontAF_store);
...
static inline int Register_AF_CharDrv(void)
{
	...
	//<2015/12/18-simonlin, Create device attribute for OIS On/Off
	device_create_file(vcm_device, &dev_attr_OIS_Switch);
	
	device_create_file(vcm_device, &dev_attr_OIS_Shutter);

	device_create_file(vcm_device, &dev_attr_OIS_Movie);

	device_create_file(vcm_device, &dev_attr_FrontAF);
	//>2015/12/18-simonlin
