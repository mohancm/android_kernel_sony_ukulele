...
// <<< 2016/02/16-youchihwang. porting charger IC
...
// <<< 2016/02/16-youchihwang. porting charger IC
#define SWCHARGER_ENABLE_PIN                   100
#define GPIO_SWCHARGER_EN_PIN_TEMPORARY        (SWCHARGER_ENABLE_PIN | 0x80000000)
// >>> 2016/02/16-youchihwang. porting charger IC
				    ...
				    unsigned char SHIFT)
{
    ...
    // <<< 2016/03/20-youchihwang. Battery. print battery log
	battery_log(BAT_LOG_CRTI, "[bq25890_config_interface] write Reg[%x]=0x%x from 0x%x\n", RegNum,
...
static int bq25890_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chargin_hw_init_done = true;
        // <<< 2016/02/16-youchihwang. porting charger IC. Pulling GPIO_CHG_EN to enable mos then active BQ25896
        battery_log (BAT_LOG_CRTI, "[bq25890_driver_probe] enable GPIO 100\n");
        bq25890_chg_en (0);
        mt_set_gpio_mode (GPIO_SWCHARGER_EN_PIN_TEMPORARY, GPIO_MODE_00);
        mt_set_gpio_dir  (GPIO_SWCHARGER_EN_PIN_TEMPORARY, GPIO_DIR_OUT);
        mt_set_gpio_out  (GPIO_SWCHARGER_EN_PIN_TEMPORARY, GPIO_OUT_ONE);
        // >>> 2016/02/16-youchihwang. porting charger IC. Pulling GPIO_CHG_EN to enable mos then active BQ25896
