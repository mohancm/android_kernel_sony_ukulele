...
// <<< 2015/12/24-youchihwang. Setting charge voltage limit array
//
...
// <<< 2015/12/22-youchihwang. Setting fast charge current limit array
//
...
// <<< 2015/12/24-youchihwang. Setting input current limit array
//
// BQ25890 REG00 IINLIM[5:0]
//
const unsigned int INPUT_CS_VTH[] = {
        // <<< 2016/02/17-youchihwang. Setting input current limit array
            0,
         5000,
        // >>> 2016/02/17-youchihwang. Setting input current limit array
        10000,
        ...
        130000,
        135000,
        // <<< 2016/03/07-youchihwang. Setting input current limit array
        140000,
        145000,
        150000,
        155000
...
/* extern variable */
/* ============================================================ // */
// <<< 2015/10/02-youchihwang. Enabling absolute VINDPM under META mode
...
static unsigned int charging_hw_init(void *data)
{
        ...
        // <<< 2016/03/30-youchihwang. Battery. Start ADC conversion.
        bq25890_ADC_start(1);
        // <<< 2016/03/30-youchihwang. Battery. Start ADC conversion.
	...
	// <<< 2016/02/13-youchihwang. Battery. Safety timer. disable hardware safety timer
    ...
    // <<< 2015/12/12-youchihwang, Setting boost mode current limit 750mA. Real otg mode output current is less than 500mA if setting 500mA.
        // <<< 2016/04/28-youchihwang. Battery. OTG mode current limit
        bq25890_config_interface(bq25890_CONA, 0x0, 0x7, 0);	/* boost current limit 500 mA */
        // >>> 2016/04/28-youchihwang. Battery. OTG mode current limit
    // >>> 2015/12/12-youchihwang, Setting boost mode current limit 750mA. Real otg mode output current is less than 500mA if setting 500mA.
        // <<< 2015/12/24-youchihwang. IR default related setting
#ifdef CONFIG_MTK_BIF_SUPPORT
        bq25890_config_interface(bq25890_CON8, 0x0, 0x7, 5);	/* enable ir_comp_resistance */
    ...
    // <<< 2016/05/22-youchihwang. Battery. Add system stability. Higher voltage avoid system drop out
	bq25890_config_interface(bq25890_CON3, 0x7, 0x7, 1);	// System min voltage 3.7V
    // >>> 2016/05/22-youchihwang. Battery. Add system stability. Higher voltage avoid system drop out

	/*PreCC mode */
        // <<< 2015/12/31-youchihwang. Precharge mode. Setting precharge current limit
        //
        // Precharge mode
        // Precharge current limit (IPRECHG) : 256 mA
        //
        bq25890_config_interface (bq25890_CON5, 0x3, 0xF, 4);
        // >>> 2015/12/31-youchihwang. Precharge mode. Setting precharge current limit

        // <<< 2015/12/21-youchihwang. Setting battery precharge to fast charge threshold
        //
	// Battery precharge to fast charge threshold (BATLOWV) : 3.0 V
	//
	...
	bq25890_config_interface(bq25890_COND, 0x1, 0x1, 7);	/* vindpm vth 0:relative 1:absolute */
        // <<< 2016/05/27-youchihwang. Battery. Setting VINDPM = 4.5V
	/* absolute VINDPM = 2.6 + code x 0.1 =4.6V;K2 24261 4.452V */
...
static unsigned int charging_enable(void *data)
{
        ...
        if (KAL_TRUE == enable) {
                ...
                if (charging_get_error_state()) {
                        ...
                        bq25890_config_interface (0x0D, 0x24, 0x7F, 0x00);
                        bq25890_config_interface (0x0D, 0x01, 0x01, 0x07);
                        battery_log(BAT_LOG_CRTI, "Enter meda mode, enabling absolute VINDPM\n");
                }
                // >>> 2015/10/02-youchihwang. Enabling absolute VINDPM under META mode
        }
...
static unsigned int charging_get_vbus(void *data)
{
        ...
        array_size = GETARRAYNUM(VBUS_VTH);
        val = bq25890_get_vbus();
        battery_log(BAT_LOG_CRTI, "register VBUSV : %X\n", val);
        battery_log(BAT_LOG_CRTI, "array_size : %X\n", array_size);
        *(unsigned int *) data = 2600 + VBUS_VTH[val];
        battery_log(BAT_LOG_CRTI, "VBUS : %d V\n", *(unsigned int *) data);

        return status;
}

static unsigned int charging_set_ADC_rate(void *data)
{
        unsigned int status = STATUS_OK;

        bq25890_set_ADC_rate(*(unsigned int *) data);

        return status;
}

static unsigned int charging_set_ADC_start(void *data)
{
        unsigned int status = STATUS_OK;

        bq25890_ADC_start(*(unsigned int *) data);
...
static unsigned int charging_sw_init(void *data)
{
	...
	/*CC mode */
        // <<< 2016/02/23-youchihwang. Battery. Setting init parameters
	bq25890_config_interface(bq25890_CON4, 0x21, 0x7F, 0);	/*ICHG (0x08)512mA --> (0x21)2112mA */
	/*Vbus current limit */
	bq25890_config_interface(bq25890_CON0, 0x1C, 0x3F, 0);	 /*input current limit, IINLIM, 1500mA */
        // >>> 2016/02/23-youchihwang. Battery. Setting init parameters

        // <<< 2016/03/03-youchihwang. CV (Constant Voltage) mode. Setting termination current limit
        bq25890_config_interface(bq25890_CON5, 0x1, 0xF, 0);    //termination current limit : 128 mA
        // >>> 2016/03/03-youchihwang. CV (Constant Voltage) mode. Setting termination current limit
	...
	bq25890_config_interface(bq25890_COND, 0x13, 0x7F, 0);
        ...
        // <<< 2016/02/23-youchihwang. Battery. Setting init parameters
	bq25890_config_interface(bq25890_CON6, 0x1D, 0x3F, 2);	/* VREG=CV 4.304V */
        // >>> 2016/02/23-youchihwang. Battery. Setting init parameters

        // <<< 2016/03/02-youchihwang. Setting UVP (Under Voltage Protection) voltage
        // When charger voltage down to 4.27V, the phone happens uvp event (uvp register : 4.15)
        pmic_set_register_value(MT6351_PMIC_RG_VCDT_LV_VTH, 0x1);        
        // >>> 2016/03/02-youchihwang. Setting UVP (Under Voltage Protection) voltage
        // <<< 2016/02/22-youchihwang. Battery. OVP. ChargerIn HV detection
        // <<< 2016/04/08-youchihwang. Battery. OVP. ChargerIn HV detection
        //
        // ChargerIn HV detection threshold : 6.5V
        //
        pmic_set_register_value(MT6351_PMIC_RG_VCDT_HV_VTH, 0xa);
        // >>> 2016/04/08-youchihwang. Battery. OVP. ChargerIn HV detection
        // >>> 2016/02/22-youchihwang. Battery. OVP. ChargerIn HV detection
	...
	/*	upmu_set_rg_vcdt_hv_en(0);*/
        // <<< 2016/03/12-youchihwang. Battery. OVP. ChargerIn HV detection
	pmic_set_register_value(MT6351_PMIC_RG_VCDT_HV_EN, 1);
        // >>> 2016/03/12-youchihwang. Battery. OVP. ChargerIn HV detection
#endif

        // <<< 2016/03/30-youchihwang. Battery. Setting ADC rate
        // DMS07718028. [Ar]Build-<5929_Power off>-<LED is blinking last 20 seconds when phone power off.>
        // DMS07714264. [So] AMEA-MY: Incorrect default battery indicator during power off charging
        // If setting adc 1s continuous conversion, bq25896 REGN pin output pulse signal that blink red led
        bq25890_set_ADC_rate(0x00);
        // DMS07718028. [Ar]Build-<5929_Power off>-<LED is blinking last 20 seconds when phone power off.>
        // >>> 2016/03/30-youchihwang. Battery. Setting ADC rate
...
// <<< 2016/03/03-youchihwang. Temporary workaround for the BQ25896 IC Iterm register error.
static unsigned int charging_set_iterm(void *data)
{
        unsigned int status = STATUS_OK;
        unsigned int iterm = *(unsigned int *) data;
        
        bq25890_set_iterml(iterm);
...
charging_hw_init, charging_dump_register, charging_enable, charging_set_cv_voltage,
	    ...
	    charging_get_bif_vbat, charging_set_chrind_ck_pdn, charging_sw_init, charging_enable_safetytimer,
            ...
            // <<< 2016/03/03-youchihwang. Temporary workaround for the BQ25896 IC Iterm register error.
            // <<< 2016/03/12-youchihwang. Battery. OVP. ChargerIn HV detection
            charging_set_ta20_current_pattern, charging_set_iterm, charging_get_vbus,
            charging_set_ADC_rate, charging_set_ADC_start};
            // >>> 2016/03/12-youchihwang. Battery. OVP. ChargerIn HV detection
            // >>> 2016/03/03-youchihwang. Temporary workaround for the BQ25896 IC Iterm register error.
