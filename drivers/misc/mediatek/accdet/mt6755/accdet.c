...
char *accdet_status_string[5] = {
	...
	/*"Double_check",*/
	"Hook_switch",
	//<2015/11/30-ricehuang, [FP19378] Non-CTIA headset warning message
	"DEVICE_UNSUPPORT",
      //>2015/11/30-ricehuang
	/*"Tvout_plug_in",*/
	"Stand_by"
};
char *accdet_report_string[4] = {
	...
	"Headset_no_mic",
	/*"HEADSET_illegal",*/
	//<2015/11/30-ricehuang, [FP19378] Unsupported wired accessory
	"HEADSET_illegal",
     //>2015/11/30-ricehuang
	/* "Double_check"*/
};
	...
	case PLUG_OUT:
		...
		if (current_status == 0) {
			...
			if (current_status == 0 && show_icon_delay != 0) {
				...
				/*accdet_auxadc_switch(0);*/
				if (220 > pin_adc_value && pin_adc_value > 90) {	/*90mv   ilegal headset*///fix  DMS07715480 by oliverchen 20160418
...
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
	pmic_pwrap_write(ACCDET_DEBOUNCE0, cust_headset_settings->debounce0);
	pmic_pwrap_write(ACCDET_DEBOUNCE1, 0x3FFF);	/*2.0s->0.5s*/
	pmic_pwrap_write(ACCDET_DEBOUNCE3, cust_headset_settings->debounce3);
