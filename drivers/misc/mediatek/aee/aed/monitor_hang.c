...
static int hang_detect_thread(void *arg)
{
	...
	while (1) {
		if ((1 == hd_detect_enabled) && (FindTaskByName("system_server") != -1)) {
			...
			if (hang_detect_counter == 0) {
				LOGE("[Hang_Detect] we should triger	DB	...aee_mode=%d\n",aee_mode);
			...
			LOGE("[Hang_Detect] hang_detect disabled.   (%d)\n",aee_mode);
		}
...
void aee_kernel_RT_Monitor_api(int lParam)
{
	if (0 == lParam) {
		...
		*/
		...
		LOGE("[Hang_Detect] hang_detect enabled %d, %d\n", hd_timeout,hd_detect_enabled);
	}
}
