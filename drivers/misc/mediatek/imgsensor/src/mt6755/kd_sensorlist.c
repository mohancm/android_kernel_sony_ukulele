...
/* The following is to avoid build error of project: mt6755_fpga related //Jessy @2014/06/04 */
    ...
    #define SUPPORT_I2C_BUS_NUM1        2//<2015/9/9-kylechang, Add Main Camera Driver IMX230, Original=0>
#endif
#ifndef SUPPORT_I2C_BUS_NUM2
    #define SUPPORT_I2C_BUS_NUM2       1//<2015/9/9-kylechang, Add Main Camera Driver IMX230, Original= 2>
#endif

//<2015/10/26-zihweishen, Add Camera Key(AF:GPIO22/EINT22; Capture:GPIO24/EINT24)
...
unsigned int camera_irq = 0;

#define SENSOR_I2C_SPEED	400

//>2015/10/26-zihweishen
//Main2 support
...
//<2015/10/26-zihweishen, Add Camera Key(AF:GPIO22/EINT22; Capture:GPIO24/EINT24)
//<2014/11/07-louisliu, Modify camera key trigger behavior.
bool keyFocus = 0;
bool keyCamera = 0;
//>2014/11/07-louisliu
//>2015/10/26-zihweishen
...
int cirq_flag = 1;
static spinlock_t cirq_flag_lock;

/*1 enable,0 disable,touch_panel_eint default status, need to confirm after register eint*/
void camera_irq_enable(void)
{
	unsigned long flags;

	spin_lock_irqsave(&cirq_flag_lock, flags);

	if (cirq_flag == 0) {
		cirq_flag = 1;
		spin_unlock_irqrestore(&cirq_flag_lock, flags);
		enable_irq(camera_irq);
	} else if (cirq_flag == 1) {
		spin_unlock_irqrestore(&cirq_flag_lock, flags);
		printk("CAMERA Eint already enabled!\n");
	} else {
		spin_unlock_irqrestore(&cirq_flag_lock, flags);
		printk("Invalid irq_flag %d!", cirq_flag);
	}
...
//<2015/10/26-zihweishen, Add Camera Key(AF:GPIO22/EINT22; Capture:GPIO24/EINT24) 
//static void cam_eint24_interrupt_handler(void)
static irqreturn_t cam_eint24_interrupt_handler(unsigned irq, struct irq_desc *desc)

{
	unsigned int cam_int24_num=24;

    printk("+++KYLE+++ CamKey  interrupt EINT24  has been triggered\n");
	if (!kpd_input_dev) {
	...
	if (keyCamera == 0){
		// mt_eint_registration(cam_int24_num, TRIGGER_RISING, cam_eint24_interrupt_handler, 1);
		irq_set_irq_type(gpio_to_irq(cam_int24_num),TRIGGER_HIGH);
		//mt_eint_set_polarity(cam_int24_num,MT_EINT_POL_POS);
		input_report_key(kpd_input_dev, 212, 1); //KEY_CAMERA:212, press=1; KEY_HP:211=FOCUS
		input_sync(kpd_input_dev);
		keyCamera = 1;
	}else{
		  //mt_eint_registration(cam_int24_num, TRIGGER_FALLING, cam_eint24_interrupt_handler, 1);
		irq_set_irq_type(gpio_to_irq(cam_int24_num), TRIGGER_LOW);
		//mt_eint_set_polarity(cam_int24_num,MT_EINT_POL_NEG);
		input_report_key(kpd_input_dev, 212, 0); //KEY_CAMERA:212, press=1; KEY_HP:211=FOCUS
		input_sync(kpd_input_dev);
		keyCamera = 0;
	}
#endif
//mt_eint_unmask(cam_int24_num);
camera_irq_enable();
return IRQ_HANDLED;
...
//static void cam_eint22_interrupt_handler(void)
static irqreturn_t cam_eint22_interrupt_handler(unsigned irq, struct irq_desc *desc)
{
	unsigned int cam_int22_num=22;

    printk("+++KYLE+++ CamKey  interrupt EINT22  has been triggered\n");
	if (!kpd_input_dev) {
		printk("KPD input device not ready\n");
		return 0;
	}
#if 1
	if (keyFocus== 0){
		irq_set_irq_type(gpio_to_irq(cam_int22_num),TRIGGER_HIGH);
		//mt_eint_set_polarity(cam_int22_num,MT_EINT_POL_POS);
		input_report_key(kpd_input_dev, 211, 1); //Volume UP:115, press=1
		input_sync(kpd_input_dev);
		keyFocus = 1;
	}else{
		irq_set_irq_type(gpio_to_irq(cam_int22_num), TRIGGER_LOW);
		//mt_eint_set_polarity(cam_int22_num,MT_EINT_POL_NEG);
		input_report_key(kpd_input_dev, 211, 0); //Volume UP:115, press=1
		input_sync(kpd_input_dev);
		keyFocus = 0;
	}
#endif
//mt_eint_unmask(cam_int22_num);
camera_irq_enable();
return IRQ_HANDLED;
...
struct of_device_id camerakey1_of_match[] = {
	{ .compatible = "mediatek,mt6755-camera_1", },
	{},
};
struct of_device_id camerakey2_of_match[] = {
	{ .compatible = "mediatek,mt6755-camera_2", },
	{},
};

static int camera24_irq_registration(void)
{
	struct device_node *node = NULL;
	//unsigned int cam_int24_num=24;
	int ret = 0;
	u32 ints[2] = { 0, 10 };
	printk("Device Tree camera_irq_registration!\n");
	node = of_find_matching_node(node, camerakey1_of_match);
	if (node)
	{
	of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
	gpio_set_debounce(ints[0], ints[1]);
	camera_irq = irq_of_parse_and_map(node, 0);
	ret=request_irq(camera_irq, (irq_handler_t) cam_eint24_interrupt_handler, TRIGGER_LOW,"CAMERA_KEY-eint", NULL);
		 if (ret > 0) 
       		 {
          	  ret = -1;
           	 printk("camera request_irq IRQ LINE NOT AVAILABLE!.");
	   	 return -1;
        	}
	}
    return ret;
...
static int camera22_irq_registration(void)
{
	struct device_node *node = NULL;
	//unsigned int cam_int22_num=22;
	int ret = 0;
	u32 ints[2] = { 0, 10 };
	printk("Device Tree camera_irq_registration!");
	node = of_find_matching_node(node, camerakey2_of_match);
	if (node)
	{
	of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
	gpio_set_debounce(ints[0], ints[1]);
	camera_irq = irq_of_parse_and_map(node, 0);
	ret=request_irq(camera_irq, (irq_handler_t) cam_eint22_interrupt_handler, TRIGGER_LOW,"CAMERA_KEY-eint", NULL);
		 if (ret > 0) 
       		 {
          	  ret = -1;
           	 printk("camera request_irq IRQ LINE NOT AVAILABLE!.");
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* set I2C clock rate */
    g_pstI2Cclient->timing = SENSOR_I2C_SPEED;/* 400 */
...
static int CAMERA_HW_i2c_probe2(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* set I2C clock rate */
    g_pstI2Cclient2->timing = SENSOR_I2C_SPEED;/* 400 */
...
static int CAMERA_HW_probe(struct platform_device *pdev)
{
	...
	/*unsigned int cam_int24_num=24;
	unsigned int cam_int22_num=22;
	unsigned int  edge_sensitive=0;
	unsigned int  lel_sensitive=1;*/
	spin_lock_init(&cirq_flag_lock);
	printk("+++KYLE+++ CAMERA_HW_probe START!!\n");
	//	<2014/09/24-kylechang,Android L must remove GPIO setting, or Reboot>
	// Setup Interrupt Pin
	//irq_set_irq_type(cam_int24_num, edge_sensitive);
	// mt_eint_set_sens(cam_int24_num, edge_sensitive);
	 //gpio_set_debounce(cam_int24_num,10);
	 //mt_eint_set_hw_debounce(cam_int24_num,10);
	 // [Lavender/Cosmos][Keypad][akenhsu] Fix Focus key will keep down even after hands released 20150317 BEGIN
	 // mt_eint_registration(cam_int24_num, TRIGGER_FALLING, cam_eint24_interrupt_handler, 1);
	 //request_irq(camera_irq, (irq_handler_t) cam_eint24_interrupt_handler, TRIGGER_LOW,"CAMERA_KEY-eint", NULL);
	 camera24_irq_registration();
	 //mt_eint_registration(cam_int24_num, TRIGGER_LOW, cam_eint24_interrupt_handler, 0);
	 // [Lavender/Cosmos][Keypad][akenhsu] 20150317 END
	...
	/*	<2014/09/24-kylechang,Android L must remove GPIO setting, or Reboot>
	mt_set_gpio_mode(GPIO138, GPIO_MODE_04);
	mt_set_gpio_dir(GPIO138, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO138, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO138, GPIO_PULL_UP);
	*/
	//irq_set_irq_type(cam_int24_num, edge_sensitive);
	//mt_eint_set_sens(cam_int22_num, edge_sensitive);
	//gpio_set_debounce(cam_int22_num,10);
	//mt_eint_set_hw_debounce(cam_int22_num, 10);
	// [Lavender/Cosmos][Keypad][akenhsu] Fix Focus key will keep down even after hands released 20150317 BEGIN
	// mt_eint_registration(cam_int24_num, TRIGGER_FALLING, cam_eint24_interrupt_handler, 1);
	//request_irq(camera_irq, (irq_handler_t) cam_eint22_interrupt_handler, TRIGGER_LOW,"CAMERA_KEY-eint", NULL);
	camera22_irq_registration();
	//mt_eint_registration(cam_int22_num, TRIGGER_LOW, cam_eint22_interrupt_handler, 0);
	// [Lavender/Cosmos][Keypad][akenhsu] 20150317 END
	//mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_LOW, tpd_eint_interrupt_handler, 1);					
	//mt_eint_unmask(cam_int22_num);
	mdelay(10);
	...
	printk("+++KYLE+++ CAMERA_HW_probe END!!\n");
#endif
	//>2015/10/26-zihweishen
