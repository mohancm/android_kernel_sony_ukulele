...
#include "imx230Bmipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "IMX230B_camera_sensor"
#define LOG_1 printk("IMX230B,MIPI 4LANE\n")
...
kal_bool Is_longExp_1_B=0;
//>2015/11/02-kylechang

//
#define BYTE unsigned char
static BOOL read_spc_flag = FALSE;
static BYTE imx230B_SPC_data[352]={0};
static BYTE eeprom_versionB[2]={0};
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX230B_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define IMX230BMIPI_MaxGainIndex (115)
kal_uint16 IMX230BMIPI_sensorGainMapping[IMX230BMIPI_MaxGainIndex][2] ={	
	{64 ,0	},
	{65 ,8	},
	{66 ,16 },
	{67 ,25 },
...
#define IMX230BMIPI_MaxGainIndex2 (152)
kal_uint16 IMX230BMIPI_sensorGainMapping2[IMX230BMIPI_MaxGainIndex2][2] ={
	{72,0x0120},
	{80,0x0140},
	{88,0x0160},
	{96,0x0180},
...
/*static kal_uint32 imx230B_ATR(UINT16 DarkLimit, UINT16 OverExp)
{
...
//	int version = 0;
	...
	read_eeprom_versionB(eeprom_versionB);
	//printk("[Rick] After, get_sensor_version 0x%x \n", eeprom_versionB[0]);
	/* NEW version //
	New LiteOn: [0]:0xc, [1]:0x1
	Truly: [0]:0x1, [1]:0x2
	*/
	if(eeprom_versionB[0] == 0xc && eeprom_versionB[1] == 0x1){ // new LiteOn
		version = 1;
	}
	else if(eeprom_versionB[0] == 0x1 && eeprom_versionB[1] == 0x2){ //Turly
		version = 1;
	}
	else{ // old LiteOn
		version = 0;
...
//
static void imx230B_apply_SPC(void)
{
	unsigned int start_reg = 0x7c00;
	int i;
	//printk("[Rick] imx230B_apply_SPC A \n");
	if(read_spc_flag == FALSE)
	{
		read_imx230B_SPC(imx230B_SPC_data);
		read_spc_flag = TRUE;
		//printk("[Rick] imx230B_apply_SPC B \n");
		return;
	}

	for(i=0;i<352;i++)
	{
		write_cmos_sensor(start_reg, imx230B_SPC_data[i]);
		//printk("[Rick] imx230B_apply_SPC C \n");	
		start_reg++;
	}		
}
//
...
{
	kal_uint8 iI;
    printk("[IMX230BMIPI]enter IMX230BMIPIGain2Reg function\n");
    for (iI = 0; iI < IMX230BMIPI_MaxGainIndex; iI++) 
	{
		if(gain < IMX230BMIPI_sensorGainMapping[iI][0])
		{                
			return IMX230BMIPI_sensorGainMapping[iI][1];       
		}
			

    }
	if(iI != IMX230BMIPI_MaxGainIndex)
	{
    	if(gain != IMX230BMIPI_sensorGainMapping[iI][0])
    	{
        	 printk("Gain mapping don't correctly:%d %d \n", gain, IMX230BMIPI_sensorGainMapping[iI][0]);
    	}
	}
	printk("exit IMX230BMIPIGain2Reg function\n");
    return IMX230BMIPI_sensorGainMapping[iI-1][1];
}
...
static kal_uint16 gain2reg2(const kal_uint16 gain)
{
    ...
    for (i = 0; i < IMX230BMIPI_MaxGainIndex2; i++) { 
    	if(gain <= IMX230BMIPI_sensorGainMapping2[i][0])
		{                
			return IMX230BMIPI_sensorGainMapping2[i][1];       
		}
    	
	}
    printk("Gain mapping don't correctly:%d index=%d \n", gain, i);
    return IMX230BMIPI_sensorGainMapping2[i-1][1];
}
...
{
	 printk("IMX230B sensor_init start\n"); 
	//DPC2D Setting
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
	printk("[Rick]get_imgsensor_id imx230B, Enter \n");
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
            *sensor_id = return_sensor_id(); //return 0x0230
			printk("[Rick][imx230B][get_imgsensor_id] return sensor id: 0x%x\n", *sensor_id);

			//new version for LiteOn
			if((*sensor_id == 0x0230) && (get_sensor_version() == 1)) //if new version of LiteOn, change sensor_id to 0x0231
			{
				printk("[Rick][imx230B][get_imgsensor_id] new LiteOn version, change sensor_id to 0x0231 \n");
				*sensor_id = 0x0231;
			}
			//new version for LiteOn
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("[Rick][imx230B][get_imgsensor_id] i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
			}
            printk("[Rick][imx230B][get_imgsensor_id] Read sensor_id fail, write id: 0x%x, sensor_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
        ...
        *sensor_id = 0xFFFFFFFF;
		printk("[Rick][imx230B][get_imgsensor_id] ERROR_SENSOR_CONNECT_FAIL, sensor_id: 0x%x\n", *sensor_id);
        return ERROR_SENSOR_CONNECT_FAIL;
    }
	printk("[Rick][imx230B][get_imgsensor_id] end \n");
    return ERROR_NONE;
}
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			 sensor_id = return_sensor_id();//return 0x0230
			 printk("[Rick][imx230B][open] return sensor id: 0x%x\n", sensor_id);
			 if((sensor_id == 0x0230) && (get_sensor_version() == 1))//if new verion of LiteOn, change sensor id to 0x0231
			 {
			 	printk("[Rick][imx230B][open] new LiteOn version, change sensor_id to 0x0231 \n");
				sensor_id = 0x0231;
				//return ERROR_SENSOR_CONNECT_FAIL;
			 }
			 //new version for LiteOn
             if (sensor_id == imgsensor_info.sensor_id) { //check if sensor_id is equal to 0x0231
                printk("[Rick][imx230B][open] sensor_id: 0x%x\n", sensor_id);
				break;
			 }
            printk("[Rick][imx230B][open] Read sensor_id fail, write id: 0x%x, sensor_id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id){
			printk("[Rick][imx230B][open] sensor_id:0x%x == imgsensor_info.sensor_id:0x%x\n", sensor_id, imgsensor_info.sensor_id);
            break;
		}
        retry = 2;
    }
    if (imgsensor_info.sensor_id != sensor_id){
		printk("[Rick][imx230B][open] ERROR_SENSOR_CONNECT_FAIL \n");
        return ERROR_SENSOR_CONNECT_FAIL;
	}
	printk("[Rick][imx230B][open] before init, sensor_id:0x%x \n", sensor_id);
    /* initail sequence write in  */
    sensor_init();
	imx230B_apply_SPC();//20160428, RickLiu
	printk("[Rick] imx230B Init Done! \n");
    spin_lock(&imgsensor_drv_lock);
...
{
    printk("imx230Bmipi_sensor preview E\n");
...
static kal_uint32 imx230B_awb_gain(SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{ 
    UINT32 rgain_32, grgain_32, gbgain_32, bgain_32;
	//LOG_INF("imx230B_awb_gain\n");
    ...
    printk("[imx230B_awb_gain] ABS_GAIN_GR:%d, grgain_32:%d\n", pSetSensorAWB->ABS_GAIN_GR, grgain_32);
    printk("[imx230B_awb_gain] ABS_GAIN_R:%d, rgain_32:%d\n", pSetSensorAWB->ABS_GAIN_R, rgain_32);
    printk("[imx230B_awb_gain] ABS_GAIN_B:%d, bgain_32:%d\n", pSetSensorAWB->ABS_GAIN_B, bgain_32);
    printk("[imx230B_awb_gain] ABS_GAIN_GB:%d, gbgain_32:%d\n", pSetSensorAWB->ABS_GAIN_GB, gbgain_32);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_imx230B_DCC((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
            break;
		...
		case SENSOR_FEATURE_SET_AWB_GAIN:
            imx230B_awb_gain(pSetSensorAWB);
            break;
		/*END OF HDR CMD*/
		/*PDAF CMD*/
...
UINT32 IMX230B_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    IMX230B_MIPI_RAW_SensorInit    */
