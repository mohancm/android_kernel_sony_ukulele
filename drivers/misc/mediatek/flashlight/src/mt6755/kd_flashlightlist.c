...
//<2015/09/13-youchihwang. Porting flash light driver LM3644TTYFFR. Creating flashlight driver attributes
extern struct i2c_client *RT4505_i2c_client;
extern int RT4505_write_reg(struct i2c_client *client, u8 reg, u8 val);

static ssize_t front_torchmode_on_show (struct device *dev, struct device_attribute *attr, char *buf)
{
        u8 chip_register_address = 0x01;
        u8 chip_register_value   = 0x09;

        logI ("front_torchmode_on +\n");
        
        //
        // Enable Register (0x01)
        //
        // Bit 07 : TX Pin Enable
        // Bit 06 : Strobe Type
        // Bit 05 : Strobe Enable
        // Bit 04 : TORCH/TEMP Pin Enable
        // Bit 03 ~ 02 : Mode Bits // 00 : Standby // 01 : IR Drive // 10 : Torch // 11 : Flash
        // Bit 01 : LED2 Enable
        // Bit 00 : LED1 Enable
        //
...
static ssize_t front_torchmode_off_show (struct device *dev, struct device_attribute *attr, char *buf)
{
        u8 chip_register_address = 0x01;
        u8 chip_register_value   = 0x00;

        logI ("front_torchmod_off +\n");
...
static ssize_t front_flashmode_on_show (struct device *dev, struct device_attribute *attr, char *buf)
{
        u8 chip_register_address = 0x01;
        u8 chip_register_value   = 0x0D;

        logI ("front_flashmode_on +\n");
...
static ssize_t rear_torchmode_on_show (struct device *dev, struct device_attribute *attr, char *buf)
{
        u8 chip_register_address = 0x01;
        u8 chip_register_value   = 0x0A;

        logI ("rear_torchmode_on +");
...
static ssize_t rear_torchmode_off_show (struct device *dev, struct device_attribute *attr, char *buf)
{
        u8 chip_register_address = 0x01;
        u8 chip_register_value   = 0x00;

        logI ("rear_torchmode_off +\n");
...
static ssize_t rear_flashmode_on_show (struct device *dev, struct device_attribute *attr, char *buf)
{
        u8 chip_register_address = 0x01;
        u8 chip_register_value   = 0x0E;

        logI ("rear_flashmode_on +\n");
...
static DEVICE_ATTR (front_torchmode_on,  0664, front_torchmode_on_show,  NULL);
static DEVICE_ATTR (front_torchmode_off, 0664, front_torchmode_off_show, NULL);
static DEVICE_ATTR (front_flashmode_on,  0664, front_flashmode_on_show,  NULL);
static DEVICE_ATTR (rear_torchmode_on,   0664, rear_torchmode_on_show,   NULL);
static DEVICE_ATTR (rear_torchmode_off,  0664, rear_torchmode_off_show,  NULL);
static DEVICE_ATTR (rear_flashmode_on,   0664, rear_flashmode_on_show,   NULL);


static struct device_attribute *flashlight_class_attrs[] = {
        &dev_attr_front_torchmode_on,
        &dev_attr_front_torchmode_off,
        &dev_attr_front_flashmode_on,
        &dev_attr_rear_torchmode_on,
        &dev_attr_rear_torchmode_off,
        &dev_attr_rear_flashmode_on
};
...
static ssize_t flashlight_create_attr (struct device *dev)
{
        ...
        for (temp = 0; temp < num; temp++)
                if ((status = device_create_file (dev, flashlight_class_attrs[temp])))
                        break;
...
#define FLASHLIGHT_MAJOR 242
	...
	if (NULL == flashlight_device) {
        ...
        //<2015/09/14-youchihwang. Porting flash light driver LM3644TTYFFR. Creating flashlight driver attributes
