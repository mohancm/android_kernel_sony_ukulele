...
{
	union power_supply_propval propVal = {ibatmA,};
	...
	if (battery_psy)
	{
		if(c != NULL)
		{
					...
					POWER_SUPPLY_PROP_CURRENT_NOW, &ret) != 0)
			{
				...
				*c = READ_CURRENT_SIGN * ret.intval;
		}
...
static struct class_attribute qns_attrs[] = {
	...
	__ATTR(charge_current, 0644, NULL, qns_param_store),
	__ATTR(alarm, 0664, qns_param_show, qns_param_store),
	__ATTR(options, 0664, qns_param_show, qns_param_store),
	__ATTR_NULL,
};
