...
int BattThermistorConverTemp(int Res)
{
	...
	if (Res >= Batt_Temperature_Table[0].TemperatureR) {
                // <<< 2016/04/03-youchihwang. Battery. Extend battery temperature range.
                TBatt_Value = Batt_Temperature_Table[0].BatteryTemp;
        } else if (Res <= Batt_Temperature_Table[(sizeof(Batt_Temperature_Table)/sizeof(BATT_TEMPERATURE)) - 1].TemperatureR) {
                TBatt_Value = Batt_Temperature_Table[(sizeof(Batt_Temperature_Table)/sizeof(BATT_TEMPERATURE)) - 1].BatteryTemp;
                // >>> 2016/04/03-youchihwang. Battery. Extend battery temperature range.
	} else {
		RES1 = Batt_Temperature_Table[0].TemperatureR;
		TMP1 = Batt_Temperature_Table[0].BatteryTemp;
