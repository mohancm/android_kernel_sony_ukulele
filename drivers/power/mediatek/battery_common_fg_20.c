...
/* Battery type name */
#define BATTERY_TYPE_NAME_SEND "1293-8715"
#endif
...
// <<< 2016/02/14-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
static int FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable = 0;
// >>> 2016/02/14-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
...
// <<< 2016/02/14-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
///////////////////////////////////////////////////////////////////////////////////////////////
//    FP022589. Battery Swelling Mitigation for retail demo.
///////////////////////////////////////////////////////////////////////////////////////////////
static ssize_t show_FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable (struct device*dev,
                                                                                 struct device_attribute *attr,
                                                                                 char *buf)
{
        battery_log (BAT_LOG_CRTI,
                     "FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable : %d \n",
                     FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable
                    );

        return sprintf(buf, "%d\n", FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable);
}

static ssize_t store_FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable (struct device *dev,
                                                                                  struct device_attribute *attr,
                                                                                  const char *buf,
                                                                                  size_t size)
{
        if (kstrtouint(buf, 10, &FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable) == 0) {
                battery_log (BAT_LOG_CRTI,
                             "FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable : %d \n",
                             FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable
                            );
...
// <<< 2016/05/11-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
static DEVICE_ATTR(enable_llk, 0744, show_FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable, store_FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable);
// >>> 2016/05/11-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
// >>> 2016/02/14-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	bat_data->BAT_CAPACITY = BMT_status.UI_SOC2;
        // <<< 2016/04/29-youchihwang. Battery. Extend battery life test (The phase of 1% -> shutdown)
        // <<< 2016/05/26-youchihwang. Reduce the chances of battery over discharge by instant battery voltage
        // <<< 2016/08/26-youchihwang. Instant Vbat approach to shutdown voltage early.
        if (bat_data->BAT_CAPACITY <= 0 && BMT_status.bat_vol > 3400)
        // >>> 2016/08/26-youchihwang. Instant Vbat approach to shutdown voltage early.
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI,
        ...
        // <<< 2016/05/26-youchihwang. Reduce the chances of battery over discharge by instant battery voltage
        BMT_status.instant_bat_vol= bat_vol;
        // >>> 2016/05/26-youchihwang. Reduce the chances of battery over discharge by instant battery voltage
}
...
// <<< 2016/02/13-youchihwang. Battery. Safety timer
#if 1
        /* Charging OT */
        battery_log (BAT_LOG_CRTI, "youchihwang BMT_status.total_charging_time : %d, BMT_status.charging_timer : %d \n", BMT_status.total_charging_time, BMT_status.charging_timer);
...
// <<< 2016/02/21-youchihwang. Battery. Maintenance.
// <<< 2016/03/14-youchihwang. Battery. Maintenance.
extern int g_temp_status;
// >>> 2016/03/14-youchihwang. Battery. Maintenance.
static PMU_STATUS mt_battery_CheckMaintenanceTime(void)
{
        PMU_STATUS status = PMU_STATUS_OK;
        // <<< 2016/03/14-youchihwang. Battery. Maintenance.
        unsigned int maintenance_cv = 4304000;
        // >>> 2016/03/14-youchihwang. Battery. Maintenance.

        battery_log (BAT_LOG_CRTI, "mt_battery_CheckMaintenanceTime maintenance_timer : %d seconds \n", BMT_status.maintenance_timer);
        // <<< 2016/03/14-youchihwang. Battery. Maintenance.
        if (g_temp_status == TEMP_POS_10_TO_POS_45) {    //normal temperature
        // >>> 2016/03/14-youchihwang. Battery. Maintenance.
                if (BMT_status.maintenance_timer >= 10) {    // bat_full state
                        // <<< 2016/03/14-youchihwang. Battery. Maintenance.
                        //
                        // Under battery full condition
                        // > 260 hours, a new charging cycle is commenced
                        //
                        // 0   ~  60  (60) hours. CV = Vmax - 50mV
                        // 60  ~ 260 (200) hours. CV = Vmax - 100mV
                        // 260 ~ 320  (60) hours. CV = Vmax - 50mV
                        // 320 ~ 520 (200) hours. CV = Vmax - 100mV
                        // 520 ~ 580  (60) hours. CV = Vmax - 50mV
                        // 580 ~ 780 (200) hours. CV = Vmax - 100mV
                        // ............
                        //
                        if(BMT_status.maintenance_timer > MAINTENANCE_SECOND_STAGE_TIME_LIMIT)
                                BMT_status.maintenance_timer -= MAINTENANCE_SECOND_STAGE_TIME_LIMIT;
                        // >>> 2016/03/14-youchihwang. Battery. Maintenance.
                        
                        if (BMT_status.maintenance_timer <= MAINTENANCE_FIRST_STAGE_TIME_LIMIT) {
                                //
                                // Under battery full condition
                                // 0 ~ 60 hours
                                // CV = Vmax - 50mV
                                //
                                maintenance_cv = MAINTENANCE_FIRST_STAGE_CV;
                        }
                        else if (BMT_status.maintenance_timer <= MAINTENANCE_SECOND_STAGE_TIME_LIMIT) {
                                //
                                // Under battery full condition
                                // 60 ~ 260 hours
                                // CV = Vmax - 100mV
                                //
                                maintenance_cv = MAINTENANCE_SECOND_STAGE_CV;
                        }

                        battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &maintenance_cv);
                }
        }
        // <<< 2016/03/14-youchihwang. Battery. Maintenance.
        else {
                BMT_status.maintenance_timer = 0;
        }
        // >>> 2016/03/14-youchihwang. Battery. Maintenance.
...
static PMU_STATUS mt_battery_CheckCallState(void)
{
	PMU_STATUS status = PMU_STATUS_OK;
        ...
        // <<< 2016/03/16-youchihwang. Battery. Call mode
	//if ((g_call_state == CALL_ACTIVE) && (BMT_status.bat_vol > V_CC2TOPOFF_THRES))
	if ((g_call_state == CALL_ACTIVE) && (battery_meter_get_battery_voltage(KAL_TRUE) > V_CC2TOPOFF_THRES))
        // >>> 2016/03/16-youchihwang. Battery. Call mode
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	if (mt_battery_CheckChargerVoltage() != PMU_STATUS_OK) {
        ...
        // <<< 2016/02/14-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
        battery_log (BAT_LOG_CRTI, "youchihwang FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable : %d \n", FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable);
        if (FP022589_Battery_Swelling_Mitigation_for_retail_demo_enable == 1)
                if (BMT_status.UI_SOC2 >= 35) {
	...
	if (mt_battery_CheckChargingTime() != PMU_STATUS_OK) {
        ...
        // <<< 2016/02/21-youchihwang. Battery. Maintenance.
        if (mt_battery_CheckMaintenanceTime() != PMU_STATUS_OK) {
                return;
        }
        // >>> 2016/02/21-youchihwang. Battery. Maintenance.
...
static void mt_battery_notify_VBat_check(void)
{
        ...
        if (bat_vol > 4500) {
                ...
                battery_log(BAT_LOG_CRTI, "[BATTERY] send Vbat notify \n");
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_vlot(%d) > 4350mV\n", BMT_status.bat_vol);
        }
        else if (bat_vol > 4350)
		/* if (BMT_status.bat_vol > 3800) //test */
	{
	        BMT_status.over_charging_timer += 10;
                if ((BMT_status.over_charging_timer >= 60) && (get_bat_charging_current_level() != 0)) {
 		        g_BatteryNotifyCode |= 0x0008;
                        battery_log(BAT_LOG_CRTI, "[BATTERY] send Vbat notify \n");
                }
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_vlot(%d) > 4350mV\n", BMT_status.bat_vol);
        // >>> 2016/02/25-youchihwang. (Vbat) over charging protection mmi warning message
                ...
                BMT_status.over_charging_timer = 0;
		g_BatteryNotifyCode &= ~(0x0008);
	}
...
static void mt_battery_notify_VBatTemp_check(void)
{
        ...
        // <<< 2016/05/09-youchihwang. Battery. 65 celsius is high temperature protection shutdown temperature (by thermal tool).
        // 64 celsius is the high temperature warnning message temperature
	// if (BMT_status.temperature >= batt_cust_data.max_charge_temperature) {
	if (BMT_status.temperature >= HIGH_TEMPERATURE_WARNNING_MESSAGE_TEMPERATURE) {
        // >>> 2016/05/09-youchihwang. Battery.
		g_BatteryNotifyCode |= 0x0002;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too high)\n",
			    BMT_status.temperature);
	}
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
        // <<< 2016/05/11-youchihwang. Battery. -20 celsius is low temperature protection shutdown temperature (by kernel).
        // -20 celsius is the low temperature warnning message temperature
        // <<< 2016/05/23-youchihwang. Battery. -23 celsius is low temperature protection shutdown temperature (by kernel).
        // -21 celsius is the low temperature warnning message temperature
	else if (BMT_status.temperature <= LOW_TEMPERATURE_WARNNING_MESSAGE_TEMPERATURE) {
        // >>> 2016/05/23-youchihwang. Battery.
        // >>> 2016/05/11-youchihwang. Battery.
...
// <<< 2016/04/11-youchihwang. Battery. OVP. ChargerIn HV detection
#define CHARGER_CHECK_TIMES 2

//
// if charger ovp happen 2 times, show message
//
static unsigned int is_charger_ovp(void)
{
        unsigned int status;
        static unsigned char charger_ovp_count = 0;

        status = pmic_get_register_value(MT6351_PMIC_RGS_VCDT_HV_DET);
        if(status == KAL_TRUE) {
                charger_ovp_count++;
                if(charger_ovp_count >= CHARGER_CHECK_TIMES)
                        status = KAL_TRUE;
                else
                        status = KAL_FALSE;
        }
...
// <<< 2016/03/29-youchihwang. Battery. for power consumption
unsigned int vbus = 0;

battery_charging_control(CHARGING_CMD_GET_VBUS, &vbus);

// <<< 2016/03/30-youchihwang. Battery. OVP. ChargerIn HV detection
// <<< 2016/04/11-youchihwang. Battery. OVP. ChargerIn HV detection
battery_log(BAT_LOG_CRTI, "[BATTERY] VBus : %d, batt_cust_data.v_charger_max : %d, RGS_VCDT_HV_DET : %X, RG_VCDT_HV_VTH : %X \n",
        vbus,
        batt_cust_data.v_charger_max,
        pmic_get_register_value(MT6351_PMIC_RGS_VCDT_HV_DET),
        pmic_get_register_value(MT6351_PMIC_RG_VCDT_HV_VTH)
        );
// >>> 2016/04/11-youchihwang. Battery. OVP. ChargerIn HV detection
// >>> 2016/03/30-youchihwang. Battery. OVP. ChargerIn HV detection
// >>> 2016/03/29-youchihwang. Battery. for power consumption
...
//#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
//        // <<< 2016/03/12-youchihwang. Battery. OVP. ChargerIn HV detection
//	if (vbus > batt_cust_data.v_charger_max) {
//#else
//	if (vbus > v_charger_max) {
//#endif
        // <<< 2016/04/11-youchihwang. Battery. OVP. ChargerIn HV detection
        if (is_charger_ovp()) {    // charger voltage > charger hight voltage limit
        // >>> 2016/04/11-youchihwang. Battery. OVP. ChargerIn HV detection
// >>> 2016/03/30-youchihwang. Battery. OVP. ChargerIn HV detection

		g_BatteryNotifyCode |= 0x0001;
		battery_log(BAT_LOG_CRTI, "[BATTERY] BMT_status.charger_vol(%d) > %d mV\n",
			    vbus, batt_cust_data.v_charger_max);
        // >>> 2016/03/12-youchihwang. Battery. OVP. ChargerIn HV detection
	} else {
	        // <<< 2016/03/12-youchihwang. Battery. Test
	        battery_log(BAT_LOG_CRTI, "[BATTERY] cancel battery vcharger notify \n");
		g_BatteryNotifyCode &= ~(0x0001);
                // >>> 2016/03/12-youchihwang. Battery. Test
	}
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
		/* ignore default rule */
                // <<< 2016/05/10-youchihwang. Battery. Low battery temperature protection shtudown. -20 celsius shtudown
                // <<< 2016/05/23-youchihwang. Battery. Low battery temperature protection shtudown. -23 celsius shtudown
                if (BMT_status.temperature <= LOW_BATTERY_TEMPERATURE_SHUTDOWN_TEMPERATURE) {
                // >>> 2016/05/23-youchihwang. Battery. Low battery temperature protection shtudown. -23 celsius shtudown
					...
					battery_log(BAT_LOG_CRTI,
						    "[Battery] Tbat(%d)<=-20, system need power down.\n",
						    BMT_status.temperature);
...
#else
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
        // <<< 2016/06/23-youchihwang, Battery. MTK [FAQ15039]. Checking charger type again avoid detecting error.
...
*/
	...
	BMT_status.charger_vol = battery_meter_get_charger_voltage();
		...
		if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
			...
			mt_charger_type_detection();
                        // <<< 2016/06/23-youchihwang, Battery. MTK [FAQ15039]. Checking charger type again avoid detecting error.
                        // <<< 2016/06/30-youchihwang, Battery. MTK [FAQ15039]. Checking charger type more times
                        if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
...
*/
		...
		battery_log(BAT_LOG_CRTI, "[PE+] Cable OUT\n");
                // <<< 2016/03/12-youchihwang. Battery. OVP. ChargerIn HV detection
                batt_cust_data.v_charger_max = V_CHARGER_MAX_6V;
                // >>> 2016/03/12-youchihwang. Battery. OVP. ChargerIn HV detection
#endif
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	unsigned int charging_enable;
        ...
        #ifndef DISABLE_HW_OVP_PMIC_ENABLES_CHARGERIN_HV_DETECTION
...
#if defined(USB_CHARGER_BATTERY_CHARGING_CURRENT)
        batt_cust_data.usb_charger_battery_charging_current = USB_CHARGER_BATTERY_CHARGING_CURRENT;
#endif
// >>> 2016/03/06-youchihwang. Battery. Setting PC USB port charging parameters
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		// <<< 2016/02/14-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
		ret_device_file = device_create_file (&(dev->dev), &dev_attr_enable_llk);
		// >>> 2016/02/14-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
	...
	BMT_status.charger_exist = KAL_FALSE;	/* for default, no charger */
        ...
        // <<< 2016/07/15-youchihwang. Battery. DMS07725635. [Ar]RT-GST-<5929_Charging>-<Phone will display no battery icon when long press power key to start.>
        // KPOC get BMT_status.bat_vol after press power key to display charging icon
        // If value is 0, kpoc display low battery icon.
        // If value is > 3.45, kpoc is normal
        // The test result ===>>> 7.432050 BMT_status.bat_vol 0
        //                       24.229619 BMT_status.bat_vol 4298
        // If press power key during 7.432050 between 24.229619, phone display low battery icon
