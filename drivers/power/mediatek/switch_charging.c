...
// <<< 2016/03/12-youchihwang. Battery. Setting the charger voltage of the Quick Charging mode
PE20_PROFILE_STRUCT pe20_profile[] = {
	...
	{BATTERY_VOLT_04_300000_V/1000, VBAT4300_VBUS},
};
// >>> 2016/03/12-youchihwang. Battery. Setting the charger voltage of the Quick Charging mode
#endif
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	if (g_temp_status == TEMP_ABOVE_POS_60) {
		...
		if (batt_cust_data.high_battery_voltage_support)
            // <<< 2016/02/16-youchihwang. Setting charging voltage according to JEITA
        ...
        // <<< 2016/02/17-youchihwang. Setting charging voltage according to JEITA
        }
		...
		cv_voltage = JEITA_TEMP_BELOW_POS_0_CV_VOLTAGE;
        // >>> 2016/02/17-youchihwang. Setting charging voltage according to JEITA
	}
...
static void set_jeita_charging_current(void)
{
	...
	if (BMT_status.charger_type == STANDARD_HOST)
        ...
        // <<< 2016/03/07-youchihwang. Setting charging parameters according to JEITA
        if (g_temp_status == TEMP_ABOVE_POS_60) {
                g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
                g_temp_input_CC_value = (g_temp_input_CC_value > CHARGE_CURRENT_1550_00_MA) ? CHARGE_CURRENT_1550_00_MA : g_temp_input_CC_value; 
        }
	else if (g_temp_status == TEMP_POS_45_TO_POS_60) {
		g_temp_CC_value = (g_temp_CC_value > CHARGE_CURRENT_810_00_MA) ? CHARGE_CURRENT_810_00_MA : g_temp_CC_value;
		g_temp_input_CC_value = (g_temp_input_CC_value > CHARGE_CURRENT_1550_00_MA) ? CHARGE_CURRENT_1550_00_MA : g_temp_input_CC_value;
	}
	else if (g_temp_status == TEMP_POS_10_TO_POS_45) {
		g_temp_CC_value = (g_temp_CC_value > CHARGE_CURRENT_2112_00_MA) ? CHARGE_CURRENT_2112_00_MA : g_temp_CC_value;
		g_temp_input_CC_value = (g_temp_input_CC_value > CHARGE_CURRENT_1550_00_MA) ? CHARGE_CURRENT_1550_00_MA : g_temp_input_CC_value;
	}
	else if (g_temp_status == TEMP_POS_0_TO_POS_10) {
		g_temp_CC_value = (g_temp_CC_value > CHARGE_CURRENT_810_00_MA) ? CHARGE_CURRENT_810_00_MA : g_temp_CC_value;
	...
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
...
#else
			{
                ...
                g_temp_CC_value = batt_cust_data.usb_charger_battery_charging_current;
                // >>> 2016/03/06-youchihwang. Battery. Setting PC USB port charging parameters
			}
#endif
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	if (charging_full_check() == KAL_FALSE) {
        ...
        // <<< 2016/02/21-youchihwang. Battery. Maintenance.
                ...
                BMT_status.maintenance_timer += 10;
        }
        // >>> 2016/02/21-youchihwang. Battery. Maintenance.
...
PMU_STATUS BAT_BatteryHoldAction(void)
{
	...
	if (battery_meter_get_battery_voltage(KAL_TRUE) < TALKING_RECHARGE_VOLTAGE ||
        // >>> 2016/03/16-youchihwang. Battery. Call mode
            g_call_state == CALL_IDLE ||
            BMT_status.UI_SOC2 < 35) {
        // >>> 2016/02/14-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
		BMT_status.bat_charging_state = CHR_CC;
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
        // <<< 2016/02/17-youchihwang. Setting thermal parameters according to JEITA
